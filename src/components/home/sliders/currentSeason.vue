<template>
  <div>
    <div v-if="loaded">
        <h5 class="text-light">
            Seasonal
            <router-link to="/season/2020/spring" class="text-light">
  ...
  export default{
    ...
    methods: {
      ...
      getCurrentSeason(){
        let self=this;
        let animes=[];
        axios.get("https://api.jikan.moe/v3/season/2020/spring")
        .then(response => {
          console.log(response.data);

          let list=response.data.anime;
          ...
          self.loaded=true;
          self.animes=animes;
          sessionStorage.setItem('season',JSON.stringify(animes));
        })
        .catch(error => { console.log(error) });
      }
    },
    created(){
      if(!sessionStorage.season) {
        this.getCurrentSeason();
      }
      else {
        this.animes=JSON.parse(sessionStorage.season)
        this.loaded=true;
        this.lazy=false;
      }
    },
