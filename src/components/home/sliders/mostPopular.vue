<template>
  <div>
    <div v-if="loaded" class="mt-4">
        <h5 class="text-light">
            Most popular
            <router-link to="/top/anime/bypopularity/1" class="text-light">
        ...
        <div class="w-100 slider rounded" style="overflow-x: auto !important;overflow-y: hidden !important;height:relative;white-space: nowrap;">
            <div :key="anime.id" v-for="anime in animes" class="d-inline-block slider-card mx-1 position-relative">
                ...
                <span class="position-absolute top-2 right-2 rounded px-1 item-stat font-weight-normal text-light" :class="screen<991 ? 'font-11' : 'font-13'">
                    <i class="fas fa-users text-light" :class="screen<991 ? 'fa-sm' : 'fa-md'"></i>
                    {{anime.members || '-'}}
                </span>
    ...
    methods: {
      ...
      getMostPopular(){
        ...
        .then(response => {
          ...
          self.loaded=true;
          self.animes=animes;
          sessionStorage.setItem('mostPopular',JSON.stringify(animes));
        })
        .catch(error => { console.log(error) });
      }
    },
    created(){
      if(!sessionStorage.mostPopular) {
        let self=this;
        setTimeout(function() {
            self.getMostPopular();
        }, 1000);
        ...
        this.animes=JSON.parse(sessionStorage.mostPopular)
        this.loaded=true;
        this.lazy=false;
      }
    },
