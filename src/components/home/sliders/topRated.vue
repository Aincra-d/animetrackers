<template>
  <div>
    <div v-if="loaded" class="mt-4">
        <h5 class="text-light">
            Top rated
            <router-link to="/top/anime/1" class="text-light">
    ...
    methods: {
      ...
      getTopRated(){
        ...
        .then(response => {
          ...
          self.loaded=true;
          self.animes=animes;
          sessionStorage.setItem('topRated',JSON.stringify(animes));
        })
        .catch(error => { console.log(error) });
      }
    },
    created(){
      if(!sessionStorage.topRated) {
        let self=this;
        setTimeout(function() {
            self.getTopRated();
        }, 1000);
        ...
        this.animes=JSON.parse(sessionStorage.topRated)
        this.loaded=true;
        this.lazy=false;
      }
    },
